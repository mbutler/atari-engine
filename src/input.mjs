...
export function createInput({target=document,keys:bindings=directions,onCommand=()=>{}}={}){
 const controls=Object.entries(bindings);
 if(controls.some(([,name])=>!['left','right','up','down','action'].includes(name)))throw new RangeError('Key bindings must name left, right, up, down or action');
 const keys=new Set();let actionPending=false;
 const press=code=>{if(bindings[code]==='action'&&!keys.has(code))actionPending=true;keys.add(code);},release=code=>keys.delete(code),clear=()=>{keys.clear();actionPending=false;};
 const down=e=>{if(bindings[e.code])e.preventDefault();press(e.code);if(!e.repeat){if(e.code==='KeyP')onCommand('pause');if(e.code==='KeyR')onCommand('reset');}};
 const up=e=>release(e.code);target.addEventListener('keydown',down);target.addEventListener('keyup',up);
 return {press,release,clear,read(){const held=name=>controls.some(([key,value])=>value===name&&keys.has(key));const action=held('action')||actionPending;actionPending=false;return {x:Number(held('right'))-Number(held('left')),y:Number(held('down'))-Number(held('up')),action};},destroy(){target.removeEventListener('keydown',down);target.removeEventListener('keyup',up);clear();}};
}
