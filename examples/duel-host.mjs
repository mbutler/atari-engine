...
import {duel,RAM} from './duel.mjs';
import {createDuelInputs} from './duel-controls.mjs';
...
const cart=duel({voices:{play:(effect,options)=>voices?.play(effect,options)}});
const reset=()=>{voices?.silence();panel.press('Digit2');panel.release('Digit2');loop.setPaused(false);};
const input=createDuelInputs({onCommand:command=>command==='pause'?loop.togglePause():reset()});
...
const loop=createLoop({input,
 ...
 render:()=>{
  ...
  const mode=cart.mode===0?'Space or Enter to start':cart.winner?`${cart.winner==='left'?'Blue':'Red'} wins — Space or Enter to play again`:'First to seven';
  const text=`${cart.ram[RAM.leftScore]} : ${cart.ram[RAM.rightScore]} · ${mode} · ${cart.ram[RAM.variation]?'Fast':'Normal'} · Left ${switches.difficulty.left.toUpperCase()} / Right ${switches.difficulty.right.toUpperCase()}`;
